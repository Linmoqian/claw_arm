#!/usr/bin/env python3
"""
arm-control skill — OpenClaw 版本
===================================
SO100 机械臂控制技能，直接通过本地 SDK（飞特舵机协议）与硬件底层通信。

本脚本适配 OpenClaw AgentSkills 规范，通过 {baseDir}/main.py 方式调用。
SDK 路径解析优先级：
  1. 环境变量 CLAW_ARM_PROJECT 指向的项目根目录
  2. 相对路径自动推断（适用于项目内 skills-openclaw/ 目录）
  3. 硬编码默认路径
...
# ============================================================
#  SDK 路径解析（兼容 OpenClaw 任意部署位置）
# ============================================================

def _resolve_sdk_path() -> str:
    """
    按优先级搜索 SDK 目录：
      1. 环境变量 CLAW_ARM_PROJECT → <project>/SDK
      2. 本文件向上逐级查找含 SDK/ 的祖先目录
      3. 默认路径 D:\\project\\claw_arm
    """
    # 方式 1: 环境变量
    env_root = os.environ.get("CLAW_ARM_PROJECT")
    if env_root:
    ...
    for _ in range(8):  # 最多向上 8 级
        if (current / "SDK").is_dir():
            return str(current)
    ...
    # 方式 3: 硬编码默认
    default = Path("D:/project/claw_arm")
    if (default / "SDK").is_dir():
        return str(default)

    raise RuntimeError(
        "找不到 SDK 目录！请设置环境变量 CLAW_ARM_PROJECT 指向 claw_arm 项目根目录。\n"
        "  例如: export CLAW_ARM_PROJECT=/path/to/claw_arm\n"
        "  或:   set CLAW_ARM_PROJECT=D:\\project\\claw_arm"
    )


_PROJECT_ROOT = _resolve_sdk_path()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="SO100 机械臂控制技能 (SDK 直连) — OpenClaw 版",
